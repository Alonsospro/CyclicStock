...
function initTheme() {
  ...
  const loginToggleBtn = document.getElementById('loginThemeToggleBtn');
  ...
  if (loginToggleBtn) loginToggleBtn.addEventListener('click', toggle);
}

function updateThemeIcon(theme) {
  const icons = document.querySelectorAll('#themeToggleBtn i, #loginThemeToggleBtn i');
  icons.forEach(icon => {
...
// =============================================================================
function initAuth() {
  // Check if there is an active session in the current tab (persists across refresh on the same device/tab)
  const sessionUser = sessionStorage.getItem('cyclic_current_user');
  ...
  // If no user is logged in (fresh tab or shared link), show direct login screen
  if (!state.currentUser) {
    updateUserProfileUI();
  ...
  // User is already logged in in this tab: keep active session across refresh
  hideLoginScreen();
  updateUserProfileUI();
  applyRolePermissions();
  loadAppConfig().then(() => {
...
function populateCentrosDropdowns(centros) {
  ...
  if (loginSelect) {
    ...
    centros.forEach(c => {
      ...
      opt.value = c.codigo;
      opt.textContent = `Centro ${c.codigo} - ${c.ubicacion} (${c.nombre})`;
...
function renderCondensedInventoryTable(items) {
  ...
  items.forEach((item, index) => {
    ...
    // Camera button click -> Open Camera Capture Modal & trigger camera on any device
    cameraBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      openCameraCaptureModal(item, tr, cameraBtn, true);
    });
    ...
    damagedElem.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        ...
        const val = parseInt(damagedElem.value, 10) || 0;
        const photos = state.damagedPhotos[item.sku] || [];
        // If item has damaged units and no photo yet, open camera immediately
        if (val > 0 && photos.length === 0) {
          openCameraCaptureModal(item, tr, cameraBtn, true);
        } else {
...
// =============================================================================
// Live Camera Engine & Damaged Items Evidence System (Universal Mobile & Desktop)
// =============================================================================
...
/**
 * Automatically compress and optimize captured photos before upload
 */
async function compressImage(fileOrBlob, maxWidth = 1920, maxHeight = 1920, quality = 0.85) {
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            if (blob && (!fileOrBlob.size || blob.size < fileOrBlob.size)) {
              resolve(blob);
            } else {
              resolve(fileOrBlob);
            }
 ...
 */
function openCameraCaptureModal(item, rowElem, cameraBtn, autoTriggerNative = false) {
  ...
  // Check if getUserMedia is supported and in a secure context (HTTPS or localhost)
  ...
  if (isSecure && hasGetUserMedia) {
    // Attempt live video stream
    startLiveCameraStream(state.camera.facingMode);
  } else {
    // Non-HTTPS mobile network context or unsupported mediaDevices:
    // Display error/direct-action state in modal
    ...
    // On mobile devices, automatically pop the native camera hardware viewfinder
    if (autoTriggerNative || isMobileDevice()) {
      document.getElementById('cameraModalNativeCaptureInput')?.click();
    }
  }
}
...
async function takeCameraSnapshot() {
  ...
  canvas.toBlob(async (blob) => {
    ...
    state.camera.capturedBlob = compressed;
...
function retakeCameraSnapshot() {
  ...
  // If live stream is supported, restart stream, otherwise trigger native camera
  const isSecure = window.isSecureContext || location.hostname === 'localhost' || location.hostname === '127.0.0.1';
    ...
    if (rowElem) {
      ...
      if (wrapper) {
        ...
        newCamBtn?.addEventListener('click', (e) => {
          e.stopPropagation();
          openCameraCaptureModal(item, rowElem, newCamBtn, true);
        });
      }
    }
...
function initEventListeners() {
  ...
  document.getElementById('btnSwitchUser').addEventListener('click', () => {
    popover.classList.add('hidden');
    state.currentUser = null;
    sessionStorage.removeItem('cyclic_current_user');
    showLoginScreen();
  });
  ...
  document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
    ...
    const submitIcon = document.getElementById('btnLoginSubmitIcon');
    ...
    if (!centro) {
      if (alertText && alertBanner) {
        alertText.textContent = 'Por favor selecciona un Centro de Trabajo.';
    ...
    if (!username || !password) {
      if (alertText && alertBanner) {
        alertText.textContent = 'Ingresa tu usuario y contraseña de acceso.';
        alertBanner.classList.remove('hidden');
      }
      return;
    }
    ...
    try {
      ...
      // Store session in sessionStorage (persists on refresh in this device/tab, not shared to other users/links)
      sessionStorage.setItem('cyclic_current_user', JSON.stringify(data.user));
      localStorage.setItem('cyclic_active_centro', state.currentCentro);

      hideLoginScreen();
      showToast(`¡Bienvenido ${data.user.nombre || data.user.usuario}! (${data.user.cargo})`, 'success');
      sfx.scanSuccess();
      ...
      sfx.error();
      if (alertText && alertBanner) {
        alertText.textContent = err.message || 'Error de conexión o credenciales incorrectas';
        alertBanner.classList.remove('hidden');
      }
      showToast(err.message, 'error');
    } finally {
  ...
  // =========================================================================
  // LIVE CAMERA & MOBILE CAPTURE MODAL LISTENERS
  // =========================================================================
  ...
  // Native Direct Camera trigger (forces native camera app on mobile devices)
  const nativeCameraInput = document.getElementById('cameraModalNativeCaptureInput');
  ...
  document.getElementById('btnCameraDirectShoot')?.addEventListener('click', triggerNativeCamera);
  document.getElementById('btnTriggerNativeCam')?.addEventListener('click', triggerNativeCamera);

  // Gallery fallback picker (allows selecting an existing photo from storage)
  const galleryFileInput = document.getElementById('cameraModalGalleryInput');
  ...
  // Unified File Processor for both Native Camera and Gallery uploads
  const handleCameraFileChange = async (e) => {
    const input = e.target;
    if (input.files && input.files[0]) {
      const originalFile = input.files[0];
      const compressedBlob = await compressImage(originalFile);
      state.camera.capturedBlob = compressedBlob;
      ...
      document.getElementById('cameraCaptureModal')?.classList.remove('hidden');
      sfx.scanSuccess();
    }
  };

  nativeCameraInput?.addEventListener('change', handleCameraFileChange);
  galleryFileInput?.addEventListener('change', handleCameraFileChange);
...
function showLoginScreen() {
  ...
  if (centroSelect && state.currentCentro) {
